...
export default function SharedBufferGeometry(material) {
...
SharedBufferGeometry.prototype = {
    ...
    remove: function (prevIdx, idx) {
        ...
        if (this.idx.position > idx.position) {
            for (let key in this.idx) {
                ...
                var pos: any = (prevIdx[key]) * componentSize;
    ...
    addBuffer: function (copyLast) {
        ...
        var drawing: any = document.querySelector('.a-drawing');
        if (!drawing) {
        ...
        if (this.previous && copyLast) {
            ...
            let uvs: any = this.previous.attributes.uv.array;
